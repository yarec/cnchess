...
//TODO 
    ...
    QAction *backward = new QAction(QIcon(":/res/images/forward.png"), "forward", this);
    connect(backward, SIGNAL(triggered()), SLOT(forward()));
    addAction(backward);

    QAction *forward = new QAction(QIcon(":/res/images/backward.png"), "backward", this);
    connect(forward, SIGNAL(triggered()), SLOT(backward()));
    addAction(forward);

    toolbutton = new QAction(QIcon(":/res/images/go-home.png"), "test", this);
    connect(toolbutton, SIGNAL(triggered()), SLOT(newGame()));
...
void ToolBar::setChessBoard(ChessBoard *chessboard) {
...
void ToolBar::test()
{
    QString s = "fiawjeffa/fjaoweij/af w";
    QString s1 = s.split(" ").first();
    QMessageBox::StandardButton reply;
    reply = QMessageBox::information(this, tr("test"), s + "\n" +s1);
}
...
void ToolBar::backward()
{
    if(!chessboard->getPcTurn())
        chessboard->backward();
}
